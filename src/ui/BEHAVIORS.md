...
- `set_plain_state(text)` is a no-op if the text already matches
  ...
  plain text (`conversation_surface.cpp:185-199`).
- `set_conversation(them, messages)` is a no-op when `them` matches and
  ...
  `subject`/`text` per row (`conversation_surface.cpp:219-229`, `201-217`).
  Otherwise the document is rebuilt, preserving scroll as below.

### Text and bubble layout

`rebuild_document` (`conversation_surface.cpp:231-297`) writes one
`QTextBlock` per message in the caller's order:

- Alignment left (incoming) or right (outgoing) inside a centered shared
  reading column (maximum 900px). The shared lane maximum is
  `message_block_width` (`conversation_surface.cpp:49-58`): at the explicit
  narrow breakpoint (viewport below 480px) the lane is near-full, the viewport
  minus the two 12px edge gutters; otherwise the lane lives inside the centered
  reading column at the ordinary 72% ratio, with the existing 160px lower bound
  and the absolute readable cap of 640px, never wider than the column. Each
  message's block width is then content-driven: `message_content_width`
  (`conversation_surface.cpp:132-166`) measures the widest single rendered line
  of the message's own visible text (the sender · timestamp header, the
  optional subject, and every body line) with the same fonts, and the caller
  adds the existing 11px-per-side horizontal bubble padding before clamping to
  the modest 160px lower bound and the shared lane maximum
  (`conversation_surface.cpp:250-260`). A genuinely short message therefore
  owns a visibly narrower lane than a longer one in the same state. One outer
  gutter (the centered-column offset plus the 12px edge) and one inner
  remainder are derived and cross-assigned in `message_block_format`
  (`conversation_surface.cpp:60-81`) so every message keeps the shared outer
  anchor pair while incoming stays left-anchored and outgoing right-anchored,
  with 4px/30px top/bottom margins whose combined 34px text-to-text distance
  minus the two 8px vertical bubble paddings keeps at least the 14px rendered
...
- Header line `You · <timestamp>` for outgoing, `<them> · <timestamp>` for
  ...
  `st::msgServiceFg` (`conversation_surface.cpp:269-275`, `83-92`, `94-102`).
- Optional subject line at 13px Medium using `st::historyTextOutFg` /
  `st::historyTextInFg` (`conversation_surface.cpp:277-280`, `104-113`).
- Body at 14px Normal in the same out/in text colors, inserted literally — the
  ...
  (`conversation_surface.cpp:286-291`, `115-124`).

`paintEvent` (`conversation_surface.cpp:299-351`) fills the viewport with the
...
- Before a rebuild, the surface records whether the vertical scrollbar sat at
  ...
  (`conversation_surface.cpp:236-238`, `294-296`).
